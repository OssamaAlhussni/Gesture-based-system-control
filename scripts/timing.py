#!/usr/bin/env python3
"""
Used to evaluate frames and response time
"""
...
def draw_centered_text(frame, text, font_scale=1.0,
                       color=(0, 255, 0), thickness=2,
                       alpha=0.5, padding=15, line_spacing=10):
    ...
    cv2.rectangle(
        overlay,
        (text_x - padding, start_y - padding),
        (text_x + max_w + padding, start_y + total_h + padding),
        (0, 0, 0),
        -1
    )
...
def main():
    ...
    with mp_hands.Hands(
        ...
        while True:
                        ...
                        if vote != current_gesture:
                            ...
                            if gesture_start_time:
                                ...
                                if held_time >= HOLD_TIME:
                                    ...
                                    if (
                                        (vote != gesture_locked or allow_repeat)
                                        and
                                        ...
                                        gesture_locked = (
                                            vote if not action_manager.ppt_mode else None
                                        )
            ...
            print(
                f"Avg latency: {avg_latency:.2f} ms | FPS: {fps:.2f}"
            )
            # 
